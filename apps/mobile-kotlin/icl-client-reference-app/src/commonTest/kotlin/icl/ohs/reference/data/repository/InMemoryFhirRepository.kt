...
/**
 * [FhirRepository] test double for UI tests - a plain in-memory map, with none of
 * [FhirEngineRepository]'s id-resolution or reference-rewriting (fixtures are expected to already
 * carry explicit ids/references).
 */
class InMemoryFhirRepository : FhirRepository {
  ...
  override suspend fun get(resourceType: String, id: String): Resource? =
    resourcesByType[resourceType]?.get(id)

  override suspend fun all(resourceType: String): List<Resource> =
    resourcesByType[resourceType]?.values?.toList().orEmpty()
